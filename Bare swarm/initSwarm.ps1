...
New-NetFirewallRule -DisplayName "Allow Portainer" -Direction Inbound -Action Allow -Protocol UDP -LocalPort 9000 | Out-Null
...
if ([string]::Concat($result) -match "docker swarm join --token (?<Token>.+) ${ipaddress}:2377") {
    ...
    Invoke-Expression "docker network create --driver=overlay public" | Out-Null
    Start-Sleep -Seconds 10
    mkdir c:\compose
    Invoke-WebRequest -UseBasicParsing -Uri "https://raw.githubusercontent.com/tfenster/BC-Swarm/master/Bare%20swarm/docker-compose.yml" -OutFile c:\compose\docker-compose.yml
